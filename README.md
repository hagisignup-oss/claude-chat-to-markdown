...
- **Coworkセッション（`claude.ai/cowork/...`）は未対応です。** 通常のチャット画面（`claude.ai/chat/...`）とはDOM構造が異なり（`data-index`が会話全体での絶対位置ではなく読み込み範囲内の相対位置になっている、ツール呼び出しの進行状況表示やファイル配信カードが発言と同じ行として扱われる、等）、そのまま保存すると内容の重複・順序の乱れ・UIテキストの混入が発生します。Coworkセッションを保存すると壊れたMarkdownが出力されるので、現時点では通常のチャット画面でのみお使いください。
